...
void Hashtable::printStats() {
  cout << "Average probes/insert = " << probeRate() << " = " << _totalProbes
       << "/" << _numInserts;
  cout << ", size = " << _size;
...
int Hashtable::hash(int k) { return k % _size; }

int Hashtable::hash2(int k, int size) {
  int n;
...
int Hashtable::hash3(int k) { return 149 - (k % 149); }
void Hashtable::qinsert(int k) {
  ...
  _numFailures += 1;
  cout << "Warning: qinsert(" << k
       << ") found no EMPTY slot, so no insert was done." << endl;
}
