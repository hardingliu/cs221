...
Node *createNode(KType key, VType value, Node *left = NULL,
                 Node *right = NULL) {
...
int getHeight(Node *node) { return (node != NULL) ? (node->height) : -1; }
...
void updateStats(VType value, double lavg, double lvar, int lcount, double ravg,
                 double rvar, int rcount, double &avg, double &var,
                 int &count) {
  double delta, m;
  count = lcount + rcount;
  if (count > 0) {
...
void findStatsSequential(Node *root, double &avg, double &var, int &count) {
  ...
  updateStats(root->value, lavg, lvar, lcount, ravg, rvar, rcount, avg, var,
              count);
}
...
void usage(const char *program) {
  ...
  std::cout << "\tPrints the mean and std deviation of the distribution of "
               "words and the number of unique words in the named file."
            << std::endl;
}

// Run REPETITIONS trials of either sequential or parallel findStats
// and reports the time in seconds that this takes.
void findStatsAndReport(
    std::function<void(Node *, double &, double &, int &)> f, Node *tree,
    const char *type) {
  double avg, var;
  int count;
