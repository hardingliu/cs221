...
int numNodes( Node* root ) {
  ...
  if (root == NULL) {
    ...
    return numNodes(root->left) + 1 + numNodes(root->right); // comment out this line
}
...
int numLeaves( Node* root ) {
  ...
  if (root == NULL)
    ...
    return numLeaves(root->left) + numLeaves(root->right); // comment out this line
}
...
int depth( Node* root, Node* x ) {
  ...
  if (root->key == x->key)
    ...
    return depth(root->right, x) + 1; // comment out this line
  else
