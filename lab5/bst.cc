...
bool delete_node(Node *&root, int key) {
  ...
  // case 3: target has two children
    ...
    Node *nparent = find_parent(root, result);
    if (parent != NULL) {
      if (nparent == target) {
...
int numNodes(Node *root) {
  ...
  if (root == NULL) {
    ...
    return numNodes(root->left) + 1 +
           numNodes(root->right); // comment out this line
}
...
int numLeaves(Node *root) {
  ...
  if (root == NULL)
    ...
    return numLeaves(root->left) +
           numLeaves(root->right); // comment out this line
}
...
int height(Node *x) {
  ...
  if (x == NULL)
    ...
    return std::max(height(x->left) + 1, height(x->right) + 1);
}
...
void runMain(Node *&tree, std::vector<int> keys) {
  ...
  for (unsigned int i = 0; i < keys.size(); ++i) {
    std::cout << "height(" << keys[i] << ") = " << height(find(tree, keys[i]))
              << std::endl;
    std::cout << "depth(" << keys[i]
              << ") = " << depth(tree, find(tree, keys[i])) << std::endl;
  }
  std::cout << std::endl;
...
std::vector<int> heights(heightArr,
                         heightArr + sizeof(heightArr) / sizeof(int));
...
public:
  std::vector<int> q;
  void visit(Node *node, int level) { q.push_back(node->key); }
};
...
void runTests(Node *&tree, std::vector<int> keys) {
  ...
  std::string d("depth");
  for (unsigned int i = 0; i < keys.size(); ++i) {
    unit.assertEquals(h + "(" + std::to_string(keys[i]) + ")", heights[i],
                      height(find(tree, keys[i])));
    unit.assertEquals(d + "(" + std::to_string(keys[i]) + ")", depths[i],
                      depth(tree, find(tree, keys[i])));
  }
  ...
  // delete root with 2 children, left child of root is not predecessor,
