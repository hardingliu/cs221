...
void list_test(string msg, Node *actual, initializer_list<int> expect) {
  ...
  if (act == exp)
    return;
  cerr << "*ERROR* " << vectorStr(act) << " != (expected) " << vectorStr(exp)
       << "\n";
}
...
int main() {
  ...
  while (list3)
    delete_last_element(list3);

  list3 = interleave(list2, list2);

  list_test("<L> interleave(list2,list2)", list3,
...
#ifdef _WIN32
  system("PAUSE"); // needed for windows to prevent the program from terminating
                   // and the command window to close
#endif
